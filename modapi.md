...
```typescript
window.modAPI.utils.calculateCraftingOvercrit(
  critChance: number,
  critMultiplier: number,
  overcrit: number,
): { multiplier: number; critCount: number; didCrit: boolean }
```

Calculate the crit multiplier for perfection and completion rolls, including overcrit re-rolls. Returns the total multiplier, the number of crits that fired (for logging or display), and whether any crit occurred at all.

- **`critChance`** — Total crit chance (can exceed 100%; excess is converted to bonus multiplier at a 1:3 ratio)
- **`critMultiplier`** — Base crit multiplier as a percentage (e.g., 200 for 2x)
- **`overcrit`** — Overcrit percentage. On each successful crit, rolls again at `floor(currentChance * overcrit / 100)` up to 9 additional times. Each extra crit adds linearly to the multiplier. Capped at 90%.

```typescript
const result = window.modAPI.utils.calculateCraftingOvercrit(150, 200, 50);
if (result.didCrit) {
  console.log(`Rolled ${result.critCount} crits for a ${result.multiplier}x multiplier`);
}
```
